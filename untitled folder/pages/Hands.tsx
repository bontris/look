...
import Store from "./../services/Hands";
                        ...
                        dash={[
                            {
                                ...
                                view: (item: any, lock: boolean) => (
                                    <Button
                                        ...
                                        onClick={(event) => {
                                            navigate(`/equipo/edit/${item.hash}`);
                                        }}>
                                        Detalles
                                    </Button>
                                )
                        ...
                        data={[
                            {
                                ...
                                cast: (item: any) => (
                                    <Stack
                                        ...
                                        direction="row">
                                        <Avatar
                                            src={(item.icon && `${BACK}/snaps/${item.icon}`)}
                                            sx={(theme) => ({
                                                width: 48,
                                                height: 48,
        ...
        const view:any = useMemo(() => (
            {
                data: [
                    ...
                    },
                    {
                        ...
                        list: [
                            {item: 1, text: 'Agente'},
                            {item: 2, text: 'Abogado Líder'},
                            {item: 3, text: 'Abogado Marcas'}
                        ]
                    },
                    {
                        type: 'text',
                    ...
                    },
                    {
                        ...
                        name: 'link',
                        text: 'Empleado',
                        list: session.heap?.head?.map((next: any) => ({...next, item: next.item, text: `${next.name} ${next.last}`, hint: next.mail})) ?? [],
                    ...
                    },
                    {
                        ...
                        name: 'note',
                        text: 'Comentarios',
                        hint: 'Comentarios sobre el empleado'
                    }
                ]
            }
        ), [session.heap]);
                            ...
                            task: async (form: User) => {
                                ...
                                Store.make(form, (done: boolean, data: any) => {
                                    if (done) {
                                        ...
                                        setAlert((data?.text ?? 'El empleado fue creado correctamente.'), 'success');
                                    } else {
                                        setData((last) => ({...last, lock: false, fail: data?.list ?? {}}));

                                        setAlert((data?.text ?? 'El empleado no pudo ser creado correctamente.'), 'error');
                                    }
                                });
                            }
                        }}
        ...
        const view:any = useMemo(() => (
            {
                data: [
                    ...
                    },
                    {
                        ...
                        name: 'link',
                        text: 'Empleado',
                        list: session.heap?.head?.map((next: any) => ({item: next.item, text: `${next.name} ${next.last}`, hint: next.mail})) ?? []
                    },
                    {
                        type: 'text',
                        size: 'half',
                        ...
                        <Form
                            ...
                            save={{
                                ...
                                task: (form: any) => {
                                    ...
                                    Store.save(`${item}`, {...form, icon: form.icon instanceof File ? form.icon : null}, (done: boolean, data: any) => {
                                        if (done) {
                                            navigate('/equipo');

                                            setAlert((data?.text ?? 'El empleado fue actualizada correctamente.'), 'success');
                                        } else {
                                            setData((last) => ({...last, lock: false, fail: data?.list ?? {}}));

                                            setAlert((data?.text ?? 'El empleado no pudo ser actualizado correctamente.'), 'error');
                                        }
                                    });
                                }
                            }}
