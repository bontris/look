...
@section('page')
						...
						<v-list-item-content>
							...
							<v-breadcrumbs
								...
								:items="[{text: 'Inicio', href: '{{route('dash')}}', disabled: false},
										 {text: 'Variables', href: '{{route('team.items')}}', disabled: true}]">
							</v-breadcrumbs>
							</v-list-item-title>
						</v-list-item-content>
					</v-list-item>
							...
							<v-sheet v-else>
								<v-list>
									<v-list-item>
										<v-list-item-content>
											<v-list-item-title class="title">
												@{{pick ? 'Editar variable' : 'Nueva variable'}}
											</v-list-item-title>
										</v-list-item-content>
									</v-list-item>
								</v-list>
                                ...
                                <v-row>
                                    <v-col cols="6">
                                        <v-select
                                            ...
                                            item-value="item"
                                            item-text="text"
                                            :items="[{item: 1, text: 'Entero'},
                                                     {item: 2, text: 'Flotante'},
                                                     {item: 3, text: 'Cadena'},
                                                     {item: 4, text: 'Objeto'}]"
                                            v-model="form.type"
                                    ...
                                    <v-col cols="6">
                                        <v-autocomplete
                                            ...
                                            :error-messages="(fail.list.node ? [fail.list.node] : [])"
                                            :disabled="wait"
                                            hide-details="auto"
                                            filled>
                                            ...
                                            <template v-slot:item="data">
                                                <v-list-item-content>
                                                    ...
                                                    <v-list-item-subtitle>@{{data.item.card}}</v-list-item-subtitle>
                                                </v-list-item-content>
                                            </template>
                                        </v-autocomplete>
                                    </v-col>
	...
	<v-dialog v-model="wipe" width="40%" persistent outlined tile>
		<v-card>
			...
			<v-divider></v-divider>
			<v-card-text class="mt-4">¿Desea eliminar la variable <b>@{{pick ? pick.name : null}} / @{{pick ? pick.nick : null}}</b>?</v-card-text>
			<v-card-actions>
			<v-spacer></v-spacer>
	...
	<v-dialog v-model="swap" width="40%" persistent outlined tile>
		<v-card>
			<v-card-title class="headline" dark>@{{((pick && pick.lock) ? 'Habilitar variable' : 'Bloquear variable')}}</v-card-title>
			<v-divider></v-divider>
			<v-card-text class="mt-4">¿Desea @{{((pick && pick.lock) ? 'habilitar' : 'bloquear')}} la variable <b>@{{pick ? pick.name : null}} / @{{pick ? pick.code : null}}</b>?</v-card-text>
			<v-card-actions>
				<v-spacer></v-spacer>
  			...
  			var self = new Vue({
			    ...
			    methods: {
			    	load: function (take, page, find, done) {
			    		axios.get("{{route('team.items', ['task' => 'load'])}}?take=" + (take || '') + '&page=' + (page || '') +
			    			                                                                           '&find=' + (find || ''), {})
				             .then(function (data) {
			    	...
			    	open: function (task, item, data) {
			    		...
			    		if (item) {
			    			switch (task) {
			    				case 'edit':
			    				    axios.get("{{route('team.items', ['task' => 'load'])}}/" + item.hash, {})
							             .then(function (data) {
			    	...
			    	save: function (form, item) {
		    	        ...
		    	        if (item) {
				    		axios.post("{{route('team.items', ['task' => 'save'])}}/" + item.hash, data, {'X-CSRF-TOKEN': '{{csrf_token()}}'})
	                             .then(function (data) {
	                            self.wait = false;
				    		...
				    		axios.post("{{route('team.items', ['task' => 'make'])}}", data, {headers: {'X-CSRF-TOKEN': '{{csrf_token()}}'}})
	                             .then(function (data) {
			    	...
			    	lock: function (item) {
			    		axios.get("{{route('team.items', ['task' => 'lock'])}}/" + item.hash, {})
				             .then(function (data) {
				            self.swap = false;
