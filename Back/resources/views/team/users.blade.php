					...
					<v-card-text class="pa-0">
						...
						<v-sheet v-if="view == 1">
							<v-data-table
								:headers="none(tiny, [
									{sortable: false},
									{text: 'Avatar', width: 80, align: 'start', sortable: false},
								...
								<template v-slot:item="{item}">
									<tr>
													...
													<v-list>
														<v-list-item>
															<v-list-item-avatar
																...
																v-on="on">
																<v-badge
																	:color="['grey', 'green', 'orange', 'red'][item.live]"
																	bordered
																	bottom
																	dot>
																	<img
																	...
																	<span
																		...
																		v-else>
																		@{{item.name.charAt(0).toUpperCase()}}@{{item.last.charAt(0).toUpperCase()}}
																	</span>
																</v-badge>
															</v-list-item-avatar>
												...
												<v-list>
													<label>
														<v-list-item link>
															...
															<v-list-item-title>@{{item.face ? 'Cambiar avatar' : 'Agregar avatar'}}</v-list-item-title>
														</v-list-item>
													...
													<v-list-item v-if="item.face" link>
														...
														<v-list-item-title>Mostrar avatar</v-list-item-title>
													</v-list-item>
													...
													<v-list-item v-if="item.face" @click="open('wipe', item)">
														...
														<v-list-item-title>Eliminar avatar</v-list-item-title>
													</v-list-item>
												</v-list>
											</v-menu>
										</td>
										...
										<td
											...
											class="text-center">
											<div class="font-weight-medium">
												<v-chip :color="{1: 'red', 2: 'blue', 3: 'green', 4: 'black'}[item.type]" text-color="white" small>
													@{{{1: 'Administrador', 2: 'Colaborador', 3: 'Comerciante', 4: 'Visitante'}[item.type]}}
												</v-chip>
											</div>
										</td>
										<td
										...
										<td>
											...
											<v-layout v-else>
												...
												<v-menu
													...
													<v-list>
														<v-list-item v-if="item.wait" @click="open('wait', item)">
															<v-list-item-icon>
																<v-icon>mdi-lock-open</v-icon>
															</v-list-item-icon>
															<v-list-item-title>Quitar bloqueo</v-list-item-title>
														</v-list-item>
														<v-list-item link>
															<v-list-item-icon>
																<v-icon>mdi-key-variant</v-icon>
														...
														<v-list-item link>
															...
															<v-list-item-title>Enviar correo electrónico</v-list-item-title>
														</v-list-item>
													</v-list>
												</v-menu>
												<v-btn
									...
									<v-row dense>
										...
										<v-col :cols="none(tiny, 6, 12)">
											<v-select
												item-value="item"
												item-text="text"
												:items="[{item: 1, text: 'Administrador'},
														 {item: 2, text: 'Colaborador'},
														 {item: 3, text: 'Comerciante'},
														 {item: 4, text: 'Visitante'}]"
												v-model="form.type"
									...
									<v-row dense>
										...
										<v-col :cols="none(tiny, 6, 12)">
											<v-text-field
												...
												:append-icon="same(form.show, true, 'mdi-eye', 'mdi-eye-off')"
												:type="same(form.show, true, 'text', 'password')"
												:disabled="wait"
									...
									<v-row dense>
										<v-col :cols="none(tiny, 6, 12)">
											<v-text-field
												...
												autocomplete="off"
												v-model="form.cell"
												:error-messages="[fail?.cell].filter(Boolean)"
												:disabled="wait"
												filled>
											</v-text-field>
										</v-col>
        ...
        <v-card>
            <v-card-title class="pa-0">
          	...
          	<v-card-text class="mt-4">
          		¿Desea eliminar la imágen del usuario <b>@{{pick?.name}} @{{pick?.last}} / @{{pick?.code}}</b>?
          	</v-card-text>
          	<v-card-actions>
            	<v-spacer>
                </v-spacer>
	...
	<v-dialog
        ...
        <v-card>
            <v-card-title class="pa-0">
          	...
          	<v-card-text class="mt-4">
          		¿Desea eliminar el usuario <b>@{{pick?.name}} @{{pick?.last}} / @{{pick?.code}}</b>?
          	</v-card-text>
          	<v-card-actions>
            	<v-spacer>
                </v-spacer>
        ...
        <v-card>
            ...
            <v-card-text class="mt-4">
                <template v-if="same(pick?.lock, 1)">
                    ¿Desea habilitar el usuario <b>@{{pick?.name}} @{{pick?.last}} / @{{pick?.code}}</b>?
                </template>
            	<template v-else>
                    ¿Desea bloquear el usuario <b>@{{pick?.name}} @{{pick?.last}} / @{{pick?.code}}</b>?
                </template>
            </v-card-text>
            <v-card-actions>
                <v-spacer>
  			...
  			var self = new Vue({
			    ...
			    methods: {
			    	load: function (take, page, text, type, done) {
			    		axios.get("{{route('team.users', ['task' => 'load'])}}?take=" + (take || '') + '&page=' + (page || '') +
			    			                                                                           '&find=' + (text || '') +
			    			                                                                           '&type=' + (type || ''), {})
				             .then(function (data) {
			    	...
			    	open: function (task, item, data) {
			    		...
			    		if (item) {
			    			switch (task) {
			    				case 'edit':
			    				    axios.get(`{{route('team.users', ['task' => 'load'])}}/${item.hash}`, {})
							             .then(function (data) {
							            self.form = {
											lock: data.data.lock,
											test: data.data.test,
			    	...
			    	save: function (form, item) {
		    	        ...
		    	        if (item) {
				    		axios.post(`{{route('team.users', ['task' => 'save'])}}/${item.hash}`, data, {'X-CSRF-TOKEN': '{{csrf_token()}}'})
	                             .then(function (data) {
                                item.type = form.type;
				    		...
				    		axios.post("{{route('team.users', ['task' => 'make'])}}", data, {headers: {'X-CSRF-TOKEN': '{{csrf_token()}}'}})
	                             .then(function (data) {
			    	...
			    	face: function (item, snap) {
			    		...
			    		axios.post(`{{route('team.users', ['task' => 'face'])}}/${item.hash}`, data, {headers: {'X-CSRF-TOKEN': '{{csrf_token()}}'}})
                             .then(function (data) {
							item.face = data.data.file ?? null;
			    	...
			    	lock: function (item, flag) {
			    		axios.get(`{{route('team.users', ['task' => 'lock'])}}/${item.hash}?flag=${flag}`, {})
				             .then(function (data) {
			    	...
			    	drop: function (item) {
			    		axios.get(`{{route('team.users', ['task' => 'drop'])}}/${item.hash}`, {})
				             .then(function (data) {
							self.wait = false;
