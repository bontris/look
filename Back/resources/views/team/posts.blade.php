...
@section('page')
						...
						<v-list-item-content>
							...
							<v-breadcrumbs
								...
								:items="[{text: 'Inicio', href: '{{route('dash')}}', disabled: false},
										 {text: 'Artículos', href: '{{route('team.posts')}}', disabled: true}]">
							</v-breadcrumbs>
							</v-list-item-title>
						</v-list-item-content>
					</v-list-item>
							...
							<v-sheet v-else>
								<v-list>
									<v-list-item>
										<v-list-item-content>
											<v-list-item-title class="title">
												@{{pick ? 'Editar artículo' : 'Nuevo artículo'}}
											</v-list-item-title>
										</v-list-item-content>
									</v-list-item>
								</v-list>
								...
								<v-col>
									<v-data-table
				    				    :headers="[{text: 'ESTADO', align: 'start', sortable: false},
                                                   ...
                                                   {text: 'TEMA', align: 'start', sortable: false},
								...
								<v-row>
									<v-col>
                                        <v-autocomplete
                                            ...
                                            :error-messages="(fail.list.kind ? [fail.list.kind] : [])"
                                            :disabled="wait"
                                            hide-details="auto"
                                            filled>
									...
									<v-col>
										<v-text-field
											type="text"
											:label="(pick ? 'Enlace: *' : 'Enlace')"
											v-model="form.slug"
											:error-messages="(fail.list.slug ? [fail.list.slug] : [])"
											:disabled="wait"
											autocomplete="nope"
                                            hide-details="auto"
											filled>
								...
								<v-row>
									<v-col>
										<v-text-field
											...
											:error-messages="(fail.list.caption ? [fail.list.caption] : [])"
											:disabled="wait"
											autocomplete="nope"
                                            hide-details="auto"
											filled>
                                ...
                                <v-row>
									<v-col>
										<v-text-field
											...
											:error-messages="(fail.list.abstract ? [fail.list.abstract] : [])"
											:disabled="wait"
											autocomplete="nope"
                                            hide-details="auto"
											filled>
                                    ...
                                    <v-col>
                                        <v-autocomplete
                                            ...
                                            label="Autor: *"
                                            v-model="form.hand"
                                            :filter="function (item, find, text) {return ((~item.code.indexOf(find)) || (~(item.name.toLocaleLowerCase() + ' ' + item.last.toLocaleLowerCase()).indexOf(find.toLocaleLowerCase())))}"
                                            :error-messages="(fail.list.hand ? [fail.list.hand] : [])"
                                            no-data-text="No hay elementos"
                                            :disabled="wait"
                                            hide-details="auto"
                                            outlined>
	...
	<v-dialog v-model="wipe" width="40%" persistent outlined tile>
		<v-card>
			<v-card-title class="headline">Eliminar artículo</v-card-title>
			<v-divider></v-divider>
			<v-card-text class="mt-4">¿Desea eliminar el artículo <b>@{{pick ? pick.caption : null}} / @{{pick ? pick.code : null}}</b>?</v-card-text>
			<v-card-actions>
			<v-spacer></v-spacer>
	...
	<v-dialog v-model="swap" width="40%" persistent outlined tile>
		<v-card>
			<v-card-title class="headline" dark>@{{((pick && pick.lock) ? 'Habilitar artículo' : 'Bloquear artículo')}}</v-card-title>
			<v-divider></v-divider>
			<v-card-text class="mt-4">¿Desea @{{((pick && pick.lock) ? 'habilitar' : 'bloquear')}} el artículo <b>@{{pick ? pick.caption : null}} / @{{pick ? pick.code : null}}</b>?</v-card-text>
			<v-card-actions>
				<v-spacer></v-spacer>
  			...
  			var self = new Vue({
			    ...
			    methods: {
			    	load: function (take, page, find, done) {
			    		axios.get("{{route('team.posts', ['task' => 'load'])}}?take=" + (take || '') + '&page=' + (page || '') +
			    			                                                                           '&find=' + (find || ''), {})
				             .then(function (data) {
			    	...
			    	open: function (task, item, data) {
			    		...
			    		if (item) {
			    			switch (task) {
			    				case 'edit':
			    				    axios.get("{{route('team.posts', ['task' => 'load'])}}/" + item.hash, {})
							             .then(function (data) {
							            self.form = {snap: null,
                                                     lock: data.data.lock,
					             			         code: data.data.code,
			    	...
			    	save: function (form, item) {
                        ...
                        data.append('kind', (form.kind || 0));

                        data.append('hand', (form.hand || 0));
		    	        ...
		    	        if (item) {
				    		axios.post("{{route('team.posts', ['task' => 'save'])}}/" + item.hash, data, {'X-CSRF-TOKEN': '{{csrf_token()}}'})
	                             .then(function (data) {
	                            self.wait = false;
				    		...
				    		axios.post("{{route('team.posts', ['task' => 'make'])}}", data, {headers: {'X-CSRF-TOKEN': '{{csrf_token()}}'}})
	                             .then(function (data) {
			    	...
			    	lock: function (item) {
			    		axios.get("{{route('team.posts', ['task' => 'lock'])}}/" + item.hash, {})
				             .then(function (data) {
				            self.swap = false;
			    	...
			    	drop: function (item) {
			    		axios.get("{{route('team.posts', ['task' => 'drop'])}}/" + item.hash, {})
				             .then(function (data) {
				            self.wipe = false;
			    ...
			    mounted: function () {
                    axios.get("{{route('team.kinds', ['task' => 'pull'])}}", {})
			             .then(function (kinds) {
		             	axios.get("{{route('team.hands', ['task' => 'pull'])}}", {})
				             .then(function (hands) {
				            self.kinds = kinds.data.data;

			             	self.hands = hands.data.data;
