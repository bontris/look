...
@section('page')
						...
						<v-list-item-content>
							...
							<v-breadcrumbs
								...
								:items="[{text: 'Inicio', href: '{{route('dash')}}', disabled: false},
										 {text: 'Temas', href: '{{route('team.kinds')}}', disabled: true}]">
							</v-breadcrumbs>
							</v-list-item-title>
						</v-list-item-content>
					</v-list-item>
				    ...
				    <v-toolbar flat>
						...
						<v-sheet v-if="view == 1">
							<v-btn
								...
								:disabled="wait">
								<v-icon left>mdi-plus</v-icon>&nbsp;Agregar tema
							</v-btn>
						</v-sheet>
					</v-toolbar>
					<v-divider>
					...
					<v-card-text class="pa-0">
						...
						<v-sheet v-if="view == 1">
							<v-row>
								<v-col>
									<v-data-table
				    				    :headers="[{text: 'ESTADO', width: 140, align: 'start', sortable: false},
                                                   {text: 'CÓDIGO', width: 160, align: 'start', sortable: false},
			    				                   {text: 'NOMBRE', align: 'start', sortable: false},
                                                   {text: 'CREACIÓN', width: 160, align: 'end', sortable: false},
			    				                   {sortable: false}]"
								        :items="list"
	...
	<v-dialog v-model="wipe" width="40%" persistent outlined tile>
		<v-card>
			<v-card-title class="headline">Eliminar tema</v-card-title>
			<v-divider></v-divider>
			<v-card-text class="mt-4">¿Desea eliminar el tema <b>@{{pick ? pick.name : null}} / @{{pick ? pick.code : null}}</b>?</v-card-text>
			<v-card-actions>
			<v-spacer></v-spacer>
	...
	<v-dialog v-model="swap" width="40%" persistent outlined tile>
		<v-card>
			<v-card-title class="headline" dark>@{{((pick && pick.lock) ? 'Habilitar tema' : 'Bloquear tema')}}</v-card-title>
			<v-divider></v-divider>
			<v-card-text class="mt-4">¿Desea @{{((pick && pick.lock) ? 'habilitar' : 'bloquear')}} el tema <b>@{{pick ? pick.name : null}} / @{{pick ? pick.code : null}}</b>?</v-card-text>
			<v-card-actions>
				<v-spacer></v-spacer>
  			...
  			var self = new Vue({
			    ...
			    methods: {
			    	load: function (take, page, find, done) {
			    		axios.get("{{route('team.kinds', ['task' => 'load'])}}?take=" + (take || '') + '&page=' + (page || '') +
			    			                                                                           '&find=' + (find || ''), {})
				             .then(function (data) {
			    	...
			    	open: function (task, item, data) {
			    		...
			    		if (item) {
			    			switch (task) {
			    				case 'edit':
			    				    axios.get("{{route('team.kinds', ['task' => 'load'])}}/" + item.hash, {})
							             .then(function (data) {
			    	...
			    	save: function (form, item) {
		    	        ...
		    	        if (item) {
				    		axios.post("{{route('team.kinds', ['task' => 'save'])}}/" + item.hash, data, {'X-CSRF-TOKEN': '{{csrf_token()}}'})
	                             .then(function (data) {
	                            self.wait = false;
				    		...
				    		axios.post("{{route('team.kinds', ['task' => 'make'])}}", data, {headers: {'X-CSRF-TOKEN': '{{csrf_token()}}'}})
	                             .then(function (data) {
			    	...
			    	lock: function (item) {
			    		axios.get("{{route('team.kinds', ['task' => 'lock'])}}/" + item.hash, {})
				             .then(function (data) {
				            self.swap = false;
			    	...
			    	drop: function (item) {
			    		axios.get("{{route('team.kinds', ['task' => 'drop'])}}/" + item.hash, {})
				             .then(function (data) {
				            self.wipe = false;
