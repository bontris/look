        ...
        <div class="nftmax-body">
            ...
            <div class="nftmax-dsinner" v-if="same(view, 1)">
                <div class="nftmax-inner__heading">
                    <h2 class="nftmax-inner__page-title">Listado de empleados</h2>
                </div>
                <div class="nftmax-table mg-top-40">
                    <div class="nftmax-table__heading">
						<h3 class="nftmax-table__title mb-0">
                            Empleados <span class="nftmax-table__badge">@{{text(data.size)}}</span>
                        </h3>
                        ...
                        <ul  class="nav nav-tabs  nftmax-dropdown__list" id="nav-tab" role="tablist">
                            <li class="nav-item dropdown ">
                                ...
                                <ul class="dropdown-menu nftmax-sidebar_dropdown">
                                    <a class="dropdown-item list-group-item" data-bs-toggle="tab" data-bs-target="#table_1" role="tab">Abogado/a</a>
                                </ul>
                            </li>
                        </ul>
                    </div>
                                ...
                                <tbody class="nftmax-table__body">
                                    <tr v-for="item in data.list">
                                        ...
                                        <td class="nftmax-table__column-7 nftmax-table__data-7">
                                            <div class=" px-3">@{{({1: 'Abogado/a'}[item.type] ?? 'Ninguno')}}</div>
                                        </td>
                                        <td class="nftmax-table__column-7 nftmax-table__data-7">
            ...
            <div class="nftmax-dsinner" v-if="same(view, 2)">
                <div class="nftmax-inner__heading">
                    <h2 class="nftmax-inner__page-title">Crear nuevo empleado</h2>
                </div>
                <div class="nftmax__item">
                    <div class="nftmax__item-heading">
                        <h2 class="nftmax__item-title nftmax__item-title--psingle">Formularo</h2>
                        <p class="nftmax__item-text nftmax__item-text--single">Detalles de empleado.</p>
                    </div>
                    <div class="row">
                        <div class="col-12">
                                ...
                                <div class="row nftmax-pcolumn">
                                    ...
                                    <div class="col-xxl-7 col-lg-7 col-12 nftmax-pcolumn__two">
                                        <div class="nftmax__item-form--main">
                                            ...
                                            <div class="nftmax__item-form--group">
                                                <label class="nftmax__item-label">Asociado</label>
                                                <select class="nftmax__item-select" v-model="form.link" required="required">
                                                    <option value="0">Seleccione una opción</option>
                                                    <optgroup label="Compañias en Bitrix24">
                                                        <option v-for="item in base" :value="item.item">@{{item.name}} @{{item.last}}</option>
                                                    </optgroup>
                                                </select>
                                                <div style="color: red" v-if="fail?.list?.link">@{{fail?.list?.link}}</div>
                                            </div>
                                            <div class="nftmax__item-form--group">
                            ...
                            <div class="nftmax__item-button--group">
                                ...
                                <button class="nftmax__item-button--single nftmax-btn nftmax-btn__bordered bg radius nftmax-item__btn" data-bs-toggle="modal"  data-bs-target="#make">Crear empleado</button>
  			...
  			var self = new Vue({
			    ...
			    methods: {
			    	load: function (take, page, text, type, done) {
			    		axios.get("{{route('hands', ['task' => 'load'])}}?take=" + (take ?? '') + '&page=' + (page ?? '') +
			    			                                                                      '&find=' + (text ?? '') +
			    			                                                                      '&type=' + (type ?? ''), {})
				             .then(function (data) {
			    	...
			    	save: function (form) {
		    	        ...
		    	        axios.post("{{route('hands', ['task' => 'make'])}}", data, {headers: {'X-CSRF-TOKEN': '{{csrf_token()}}'}})
	                             .then(function (data) {
