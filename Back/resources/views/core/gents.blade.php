        ...
        <div class="nftmax-body">
            ...
            <div class="nftmax-dsinner" v-if="same(view, 1)">
                <div class="nftmax-inner__heading">
                    <h2 class="nftmax-inner__page-title">Listado de clientes</h2>
                </div>
                <div class="nftmax-table mg-top-40">
                    <div class="nftmax-table__heading">
						<h3 class="nftmax-table__title mb-0">
                            Clientes <span class="nftmax-table__badge">@{{text(data.size)}}</span>
                        </h3>
                        ...
                        <ul  class="nav nav-tabs  nftmax-dropdown__list" id="nav-tab" role="tablist">
                            <li class="nav-item dropdown ">
                                ...
                                <ul class="dropdown-menu nftmax-sidebar_dropdown">
                                    <a class="dropdown-item list-group-item" data-bs-toggle="tab" data-bs-target="#table_1" role="tab">Empleado</a>
                                    <a class="dropdown-item list-group-item" data-bs-toggle="tab" data-bs-target="#table_2" role="tab">Contratista</a>
                                </ul>
                            </li>
                        </ul>
                    </div>
                            ...
                            <table id="nftmax-table__main" class="nftmax-table__main nftmax-table__main-v1">
                                ...
                                <thead class="nftmax-table__head">
                                    <tr>
                                        ...
                                        <th class="nftmax-table__column-6 nftmax-table__h6">Compañía</th>
                                            ...
                                            <div class="nftmax-table__product">
                                                ...
                                                <div class="nftmax-table__product-content">
                                                    ...
                                                    <p class="nftmax-table__product-desc">
                                                        Código <a href="#">@{{item.card ?? item.code}}</a>
                                                    </p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="nftmax-table__column-7 nftmax-table__data-7">
                                            <div class="px-3">@{{item.firm}}</div>
                                        </td>
                                        <td class="nftmax-table__column-7 nftmax-table__data-7">
                                            <div class="nftmax-table__status px-3" :class="item.lock ? 'nftmax-rbcolor' : 'nftmax-gbcolor'">@{{(item.lock ? 'Inactivo' : 'Activo')}}</div>
                                        </td>
                                        <td>
                                            <a @click="open('edit', item)" class="nftmax-table__action_btn">
            ...
            <div class="nftmax-dsinner" v-if="same(view, 2)">
                <div class="nftmax-inner__heading">
                    <h2 class="nftmax-inner__page-title">@{{(pick ? 'Actualizar cliente' : 'Crear nuevo cliente')}}</h2>
                </div>
                <div class="nftmax__item">
                    <div class="nftmax__item-heading">
                        <h2 class="nftmax__item-title nftmax__item-title--psingle">Formularo</h2>
                        <p class="nftmax__item-text nftmax__item-text--single">Detalles deL cliente.</p>
                    </div>
                    <div class="row">
                        <div class="col-12">
                                ...
                                <div class="row nftmax-pcolumn">
                                    ...
                                    <div class="col-xxl-7 col-lg-7 col-12 nftmax-pcolumn__two">
                                        <div class="nftmax__item-form--main">
                                            ...
                                            <div class="nftmax__item-form--group">
                                                <label class="nftmax__item-label">Documento</label>
                                                <input class="nftmax__item-input" type="text" placeholder="Número de identificación" required="required" v-model="form.card">
                                                <div style="color: red" v-if="fail?.list?.card">@{{fail?.list?.card}}</div>
                                            </div>
                                            <div class="nftmax__item-form--group">
                                                <label class="nftmax__item-label">Nombre</label>
                                                <input class="nftmax__item-input" type="text" placeholder="Nombre del empleadoa" required="required" v-model="form.name">
                                                <div style="color: red" v-if="fail?.list?.name">@{{fail?.list?.name}}</div>
                                            </div>
                                            <div class="nftmax__item-form--group">
                                                <label class="nftmax__item-label">Apellidos</label>
                                                <input class="nftmax__item-input" type="text" placeholder="Apellidos del empleado" required="required" v-model="form.last">
                                                <div style="color: red" v-if="fail?.list?.last">@{{fail?.list?.last}}</div>
                                            </div>
                                            <div class="nftmax__item-form--group">
                                                <label class="nftmax__item-label">Empresa</label>
                                                <select class="nftmax__item-select" v-model="form.bind" required="required">
                                                    <option value="0">Seleccione una opción</option>
                                                    <option v-for="item in firm" :value="item.item">@{{item.name}} (@{{(item.card ?? item.code)}})</option>
                                                </select>
                                                <div style="color: red" v-if="fail?.list?.bind">@{{fail?.list?.bind}}</div>
                                            </div>
                                            <div class="nftmax__item-form--group">
                                            ...
                                            <div class="nftmax__item-form--group">
                                                ...
                                                <textarea class="nftmax__item-input nftmax__item-textarea" placeholder="Comentarios sobre el cliente" required="required" v-model="form.note"></textarea>
                            ...
                            <div class="nftmax__item-button--group">
                                ...
                                <button class="nftmax__item-button--single nftmax-btn nftmax-btn__bordered bg radius nftmax-item__btn" data-bs-toggle="modal"  data-bs-target="#save">@{{(pick ? 'Actualizar cliente' : 'Crear cliente')}}</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-dialog  nftmax-close__modal-close">
                    <div class="modal-content nftmax-preview__modal-content">
                        ...
                        <div class="modal-body nftmax-modal__body modal-body nftmax-close__body">
                            <div class="nftmax-preview__close">
                                <div class="nftmax-preview__close-img"><img src="assets/img/close.png" alt="#"></div>
                                <h2 class="nftmax-preview__close-title">@{{(pick ? '¿Desea actualizar el cliente?' : '¿Desea crear la nuevo cliente?')}}</h2>
                                <div class="nftmax__item-button--group">
                ...
                <div class="modal-dialog  nftmax-close__modal-close">
                    <div class="modal-content nftmax-preview__modal-content">
                        ...
                        <div class="modal-body nftmax-modal__body modal-body nftmax-close__body">
                            <div class="nftmax-preview__close">
                                <div class="nftmax-preview__close-img"><img src="assets/img/close.png" alt="#"></div>
                                <h2 class="nftmax-preview__close-title">¿Desea eliminar el cliente?</h2>
                                <div class="nftmax__item-button--group">
  		...
  		Vue.ready(function () {
  			var self = new Vue({
			    ...
			    watch: {
                    ...
                    'form.bind': (item) => {
                        if (item) {
                            axios.get(`{{route('firms', ['task' => 'list'])}}/${item}`, {})
				             .then(function (data) {
                                self.link =  data?.data ?? [];
			    ...
			    methods: {
			    	load: function (take, page, text, type, done) {
			    		axios.get("{{route('gents', ['task' => 'load'])}}?take=" + (take ?? '') + '&page=' + (page ?? '') +
			    			                                                                      '&find=' + (text ?? '') +
			    			                                                                      '&type=' + (type ?? ''), {})
				             .then(function (data) {
			    	...
			    	open: function (task, item, data) {
			    		...
			    		if (item) {
			    			switch (task) {
			    				case 'edit':
			    				    axios.get(`{{route('gents', ['task' => 'load'])}}/${item.hash}`, {})
							             .then(function (data) {
							            self.form = {
                                            lock: data.data.lock,
                                            test: data.data.test,
			    	...
			    	save: function (form, item) {
                        ...
                        if (item) {
				    		axios.post(`{{route('gents', ['task' => 'save'])}}/${item.hash}`, data, {'X-CSRF-TOKEN': '{{csrf_token()}}'})
	                             .then(function (data) {
                                $('#save').modal('hide');

                                item.firm = data.data.bind?.name;
                            ...
                            axios.post("{{route('gents', ['task' => 'make'])}}", data, {headers: {'X-CSRF-TOKEN': '{{csrf_token()}}'}})
	                             .then(function (data) {
                                $('#save').modal('hide');
                    ...
                    drop: function (item) {
			    		axios.get(`{{route('gents', ['task' => 'drop'])}}/${item.hash}`, {})
				             .then(function (data) {
                            $('#drop').modal('hide');
