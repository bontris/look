        ...
        <div class="nftmax-body">
            ...
            <div class="nftmax-dsinner" v-if="same(view, 1)">
                <div class="nftmax-inner__heading">
                    <h2 class="nftmax-inner__page-title">Listado de Validaciones de Internacionales</h2>
                </div>
                <div class="nftmax-table mg-top-40">
                    <div class="nftmax-table__heading">
						<h3 class="nftmax-table__title mb-0">
                        Validaciones de Internacionales <span class="nftmax-table__badge">@{{text(data.size)}}</span>
                        </h3>
            ...
            <div class="nftmax-dsinner" v-if="same(view, 2)">
                <div class="nftmax-inner__heading">
                    <h2 class="nftmax-inner__page-title">@{{pick?.card}}</h2>
                </div>
                <div class="nftmax-table mg-top-40">
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active" id="table_1" role="tabpanel" aria-labelledby="table_1">
                            ...
                            <table id="nftmax-table__main" class="nftmax-table__main nftmax-table__main-v1">
                                ...
                                <!-- NFTMax Table Body -->
                                <tbody class="nftmax-table__body" v-if="Boolean(pick?.data?.list?.length)">
                                    <template v-for="(list, next) in pick?.data?.list">
                                        <tr>
                                            <td style="text-align: left; background: #F3F3F3" colspan="2">
                                                Registro #@{{next + 1}}
                                            </td>
                                        </tr>
                            ...
                            <div class="nftmax-preview__close" v-else>
                                <div style="width: 100%; overflow: hidden">
                                    ...
                                    <div class="nftmax__item-form--group">
                                        <select class="nftmax__item-select" style="width: 100%" v-model="from" required>
                                        ...
                                        <div style="color: red" v-if="fail?.list?.from">@{{fail?.list?.from}}</div>
                                    </div>
                                    <div class="nftmax__item-form--group">
                                        <select class="nftmax__item-select" style="width: 100%" v-model="type" required>
                                ...
                                <div class="nftmax__item-button--group">
                                    <button class="nftmax__item-button--single nftmax-btn nftmax-btn__bordered bg radius" @click="post(from, type, card)" :disabled="wait">
                                        Consultar
                                    </button>
  			...
  			var self = new Vue({
			    ...
			    methods: {
			    	load: function (take, page, text, done) {
			    		axios.get("{{route('pasts', ['type' => 'internacionales', 'task' => 'load'])}}?take=" + (take ?? '') + '&page=' + (page ?? '') +
			    			                                                                                                   '&find=' + (text ?? ''), {})
				             .then(function (data) {
			    	...
			    	post: function (from, type, card) {
                        ...
                        axios.post("{{route('pasts', ['type' => 'internacionales', 'task' => 'post'])}}", data, {headers: {'X-CSRF-TOKEN': '{{csrf_token()}}'}})
	                             .then(function (data) {
                            $('#post').modal('hide');
                    ...
                    drop: function (item) {
			    		axios.get(`{{route('pasts', ['type' => 'internacionales', 'task' => 'drop'])}}/${item.hash}`, {})
				             .then(function (data) {
                            $('#drop').modal('hide');
                            self.wait = false;
