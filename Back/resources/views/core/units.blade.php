						...
						<v-list-item class="px-1">
							...
							<v-list-item-content class="pa-0">
								...
								<v-list-item-subtitle>
									<v-breadcrumbs
										class="pa-0"
										:items="[{text: 'Inicio', href: '{{route('dash')}}'}, {text: 'Unidades'}]">
									</v-breadcrumbs>
								</v-list-item-subtitle>
							</v-list-item-content>
						</v-list-item>
							...
							<v-sheet v-else>
								<v-list>
									<v-list-item>
										<v-list-item-content>
											<v-list-item-title>
												@{{pick ? 'EDITAR UNIDAD' : 'NUEVA UNIDAD'}}
											</v-list-item-title>
										</v-list-item-content>
									</v-list-item>
								</v-list>
									...
									<tr>
										...
										<td v-if="none(tiny)">
											...
											<div
												...
												@{{same(item.kind, 1, item.lead, item.firm)}}
											</div>
										</td>
										<td v-if="none(tiny)">
											<div class="text--primary">
												@{{none(item.step, 'Ninguna', item.step)}}
											</div>
											<v-progress-linear
												class="mt-1"
												:value="item.rate" />
							...
							<v-layout
								...
								<v-sheet>
									...
									<v-row dense>
										...
										<v-col :cols="none(tiny, 6, 12)">
											<v-select
												...
												item-text="text"
												:items="[
													{item: 1, text: 'Iniciación'},
													{item: 2, text: 'Planificación'},
													{item: 3, text: 'Ejecución'},
													{item: 4, text: 'Seguimiento'},
													{item: 5, text: 'Finalización'}
												]"
												v-model="form.rank"
											...
											<v-menu
												...
												<template v-slot:activator="{on}">
													<v-text-field
														...
														label="@lang('Fecha de iniciación')"
														:disabled="wait"
														readonly
														filled>
													</v-text-field>
										...
										<v-col :cols="none(tiny, 6, 12)">
											<v-menu
												...
												<template v-slot:activator="{on}">
													<v-text-field
														v-on="on"
														:error-messages="[fail?.stop].filter(Boolean)"
														hide-details="auto"
									...
									<v-row dense>
										<v-col :cols="none(tiny, 6, 12)">
                                            <v-autocomplete
												:items="pile.lead.list.filter((item) => same(item.sort, 2))"
												item-value="item"
												no-data-text="No hay opciones"
												label="Contratista"
												v-model="form.skip"
												...
												:error-messages="[fail?.skip].filter(Boolean)"
												:disabled="wait"
                                                clearable
												filled>
												<template v-slot:selection="data">
									...
									<v-row dense>
										<v-col :cols="none(tiny, 6, 12)">
                                            <v-autocomplete
												:items="pile.lead.list.filter((item) => same(item.sort, 1))"
												item-value="item"
												no-data-text="No hay opciones"
												label="Propietario"
												v-model="form.link"
                                    ...
                                    <v-row dense>
                                        ...
                                        <v-col :cols="none(tiny, 2, 12)">
											<v-text-field
												type="text"
												label="Apartamento / Oficina / Unidad"
												hide-details="auto"
												autocomplete="off"
												v-model="form.gate"
  		...
  		Vue.ready(() => {
			var plat = null;
  			...
  			var self = new Vue({
			    ...
			    methods: {
			    	load: (take, page, seek) => {
			    		axios.get(`{{route('core.units', ['task' => 'load'])}}?take=${take ?? ''}&page=${page ?? ''}&seek=${seek ?? ''}`, {})
				             .then((data) => {
			    	...
			    	open: (task, item, data, type, post) => {
			    		if (item) {
			    			switch (task) {
			    				case 'edit':
			    				    axios.get(`{{route('core.units', ['task' => 'load'])}}/${item.hash}`, {})
							             .then((data) => {
							            self.form = {
                                            icon: null,
                                            back: null,
			    	...
			    	save: (form, item) => {
		    	        ...
		    	        if (item) {
				    		axios.post(`{{route('core.units', ['task' => 'save'])}}/${item.hash}`, data, {'X-CSRF-TOKEN': '{{csrf_token()}}'})
	                             .then((data) => {
                                item.icon = data.data.icon ?? item.icon;
				    		...
				    		axios.post("{{route('core.units', ['task' => 'make'])}}", data, {headers: {'X-CSRF-TOKEN': '{{csrf_token()}}'}})
	                             .then((data) => {
                    ...
                    shot: (item, snap, type, post) => {
						if (post) {
							...
							axios.post(`{{route('core.units')}}/$@{{1: 'icon', 2: 'back'}[type]}/${item.hash}`, data, {headers: {'X-CSRF-TOKEN': '{{csrf_token()}}'}})
								.then((data) => {
								switch (type) {
									case 1:
					...
					move: (item, spot) => {
			    		...
			    		axios.post(`{{route('core.units', ['task' => 'move'])}}/${item.hash}`, data, {headers: {'X-CSRF-TOKEN': '{{csrf_token()}}'}})
                             .then((data) => {
			    	...
			    	lock: (item, flag) => {
			    		axios.get(`{{route('core.units', ['task' => 'lock'])}}/${item.hash}?flag=${flag}`, {})
				             .then((data) => {
			    	...
			    	drop: (item) => {
			    		axios.get(`{{route('core.units', ['task' => 'drop'])}}/${item.hash}`, {})
				             .then((data) => {
                            self.wait = false;
			    ...
			    mounted: () => {
                    setTimeout(() => {
						...
						axios.get(`{{route('core.leads', ['task' => 'pull'])}}`)
							.then((data) => {
