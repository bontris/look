    ...
    public function main (Request $request, $task = null, $item = null) {
    	if (isset($item)) {
    			...
    			switch (strtolower($task)) {
    				case 'save':
    					$validator = Validator::make($request->all(), [
							...
							'slug' => 'nullable|max:32',
							'name' => 'required|max:32',
							'hint' => 'required|max:128'
			            ], [
								...
								if ((empty(($same = DB::table('kinds')
									...
									if ((empty(($same = DB::table('kinds')
									    ...
									    return response()->json(['text' => 'Uno o mas campos del formulario no son correctos.',
															     'list' => ['slug' => 'El enlace ya existe.']], 400);
								    }
								} else {
									return response()->json(['text' => 'Uno o mas campos del formulario no son correctos.',
    		...
    		switch (strtolower($task)) {
		            ...
		            if (empty($validator->fails())) {
						if (empty(($same = DB::table('kinds')
							...
							if (empty(($same = DB::table('kinds')
								...
								if (empty(($same = DB::table('kinds')
													 ->where('hide', 0)
													 ->where('slug', ($slug = trim($request->get('slug')) ? trim($request->get('slug')) : Str::slug($request->get('name'), '-')))
													 ->first()))) {
									if (($item = DB::table('kinds')
													->insertGetId(['code' => $code,
													               'slug' => $slug,
																   'name' => trim($request->get('name')),
																   'hint' => intval($request->get('hint')),
				                  ...
				                  if (empty($item)) {
				                    $query->where(function ($query) use ($data) {
				                      $query->where('code', 'like', sprintf('%%%s%%', $data))
											...
											->orWhere('hint', 'like', sprintf('%%%s%%', $data));
				                    });
				                  } else {
				...
				default:
    				return view('/team/kinds', ['request' => $request]);
    		}
    	}
    }
}