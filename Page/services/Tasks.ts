...
export default {
    load (item: Null<number | string>, take: Null<number>, page: Null<number>, find: Null<string>, pipe: Null<string>, load: Null<string>, sort: Null<Hash<boolean>>, back: (done: boolean, data: any) => void) {
        axios.post(`${BACK}/tasks/load${(item ? `/${item}` : '')}?take=${take ?? 0}&page=${page ?? 0}&find=${(find ?? '')}`, {
            sort: Object.keys((sort ?? {})).map((name) => ({
                name: name,
                down: sort?.[name]
            }))
        }, {timeout: 900000}).then((done) => {
            back(true, done.data);
        }).catch((fail) => {
    ...
    find (item: number | string, back: (done: boolean, data: any) => void) {
        axios.get(`${BACK}/tasks/find/${item}`).then((done) => {
            back(true, done.data);
        }).catch((fail) => {
            back(false, {text: fail?.response?.data?.text ?? ((typeof fail?.response?.data == 'string') ? fail?.response?.data : 'Se presentó una excepción no esperada.')});
        });
    ...
    drop (item: number | string, back: (done: boolean, data: any) => void) {
        axios.delete(`${BACK}/tasks/drop/${item}`)
             .then(function (done) {
            back(true, done.data);
        }).catch((fail: any) => {
            back(false, {text: fail?.response?.data?.text ?? ((typeof fail?.response?.data == 'string') ? fail?.response?.data : 'Se presentó una excepción no esperada.')});
